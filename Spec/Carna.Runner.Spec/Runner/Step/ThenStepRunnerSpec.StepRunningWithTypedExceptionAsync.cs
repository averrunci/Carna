...
namespace Carna.Runner.Step
{
    ...
    class ThenStepRunnerSpec_StepRunningWithTypedExceptionAsync : FixtureSteppable
    {
        ...
        void Ex03()
        {
            ...
            Given("async next ThenStep that asserts the Exception that is thrown at WhenStep", () => NextStep = FixtureSteps.CreateThenStep<ArgumentNullException>(async exc =>
            {
                await Task.Delay(100);
                thenStepCompleted = true;
            }));
        ...
        void Ex04()
        {
            ...
            Given("async next ThenStep that asserts the Exception that is not thrown at WhenStep", () => NextStep = FixtureSteps.CreateThenStep<InvalidOperationException>(async exc =>
            {
                await Task.Delay(100);
            }));
            When("the given ThenStep is run", () =>
