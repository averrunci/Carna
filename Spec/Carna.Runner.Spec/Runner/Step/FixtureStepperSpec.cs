...
[Specification($"{nameof(FixtureStepper)} Spec")]
class FixtureStepperSpec : FixtureSteppable
{
    ...
    void Ex03()
    {
        FixtureStepResult? runningResult = default;
        FixtureStepResult? runResult = default;
        Stepper.FixtureStepRunning += (s, e) => runningResult = e.Result;
        Stepper.FixtureStepRun += (s, e) => runResult = e.Result;

        SimpleStep step = default!;
        FixtureStepResultAssertion expectedResult = default!;
        Given("SimpleStep", () =>
        {
            step = new SimpleStep();
        ...
        Then($"the result should be as follows:{expectedResult.ToDescription()}", () => runningResult != null && FixtureStepResultAssertion.Of(runningResult) == expectedResult);
    ...
    private class FixtureStepResultAssertion : AssertionObject
    {
        ...
        public static FixtureStepResultAssertion ForRunning(FixtureStep step) => new(step, true, false, false, null, FixtureStepStatus.Running);
        public static FixtureStepResultAssertion Of(FixtureStepResult result) => new(result.Step, result.StartTime.HasValue, result.EndTime.HasValue, result.Duration.HasValue, result.Exception, result.Status);
    }

    private class SimpleStep : FixtureStep
    {
        public SimpleStep() : base(string.Empty, typeof(FixtureStepperSpec), string.Empty, string.Empty, 0)
        {
        }
    }
    ...
    {
        public IFixtureStepRunner Create(FixtureStep step) => new SimpleStepRunner((SimpleStep)step);
