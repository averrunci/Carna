...
{
    Expression<Func<Exception, bool>> Assertion { get; set; } = _ => false;
    ArgumentNullException Exception { get; } = new("Parameter name", "Message");
...
",
  ...
  [failed]
    ...
    void Ex14()
    {
        ...
        Given(
            "an assertion that has 'exc.ParamName == 'Parameter' && exc.Parameter name.Length == 3 && exc.Message.Length > 0' where the type of exc is ArgumentNullException; exc.ParamName='Parameter name'; exc.Message='Message'",
            () => Assertion = CreateAssertion(exc => exc.ParamName == "Parameter" && exc.ParamName.Length == 3 && exc.Message.Length > 0)
        );
        Expect(
            @"the description should be as follows:
  [failed]
