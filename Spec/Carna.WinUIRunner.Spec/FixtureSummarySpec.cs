...
using Carna.Runner;
using Carna.WinUIRunner.AssertionObjects;

namespace Carna.WinUIRunner;

[Specification($"{nameof(FixtureSummary)} Spec")]
class FixtureSummarySpec : FixtureSteppable
{
    FixtureSummary Summary { get; } = new();

    FixtureSummaryAssertion ExpectedSummary { get; set; } = default!;
    ...
    void Ex03()
    {
        ...
        ExpectedSummary = FixtureSummaryAssertion.Of(
            0, 0, 0, 0,
            false, true, false,
            fixtureBuildingCompletedDateTime.ToString("u"), string.Empty, string.Empty
        );
        Expect($"the state should be as follows:{ExpectedSummary.ToDescription()}", () => FixtureSummaryAssertion.Of(Summary) == ExpectedSummary);
