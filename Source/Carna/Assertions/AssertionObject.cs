...
{
    private const BindingFlags AssertionPropertyBindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
    ...
    protected IEnumerable<AssertionPropertyContext> GetAssertionProperties()
        => GetType().GetProperties(AssertionPropertyBindingFlags)
            .Where(p => p.GetCustomAttribute<AssertionPropertyAttribute>() is not null || typeof(IAssertionProperty).IsAssignableFrom(p.PropertyType))
    ...
    public class StringOptions
    {
        ...
        public StringOptions(string? prefix, string? suffix, string? separator, int level) : this(_ => prefix, _ => suffix, _ => separator, level)
        {
        }
        ...
        public StringOptions ToNextLevel() => new(prefixGenerator, suffixGenerator, separatorGenerator, Level + 1);
    }
