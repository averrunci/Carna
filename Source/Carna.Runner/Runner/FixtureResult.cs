...
public class FixtureResult
{
    ...
    /// </summary>
    public IEnumerable<Exception> StepExceptions => StepResults.Select(step => step.Exception).OfType<Exception>().ToList();
    ...
    public class Builder
    {
        ...
        protected virtual FixtureStatus ResolveStatus(IEnumerable<FixtureStepStatus> statuses)
        {
            ...
            return !statusList.Any() ? FixtureStatus.Ready :
                statusList.All(status => status is FixtureStepStatus.Ready) ? FixtureStatus.Ready :
                statusList.All(status => status is FixtureStepStatus.Pending) ? FixtureStatus.Pending :
                statusList.Any(status => status is FixtureStepStatus.Failed) ? FixtureStatus.Failed :
        ...
        protected virtual FixtureStatus ResolveStatus(IEnumerable<FixtureStatus> statuses)
        {
            ...
            return !statusList.Any() ? FixtureStatus.Ready :
                statusList.All(status => status is FixtureStatus.Ready) ? FixtureStatus.Ready :
                statusList.All(status => status is FixtureStatus.Pending) ? FixtureStatus.Pending :
                statusList.Any(status => status is FixtureStatus.Failed) ? FixtureStatus.Failed :
                FixtureStatus.Passed;
        }
    }
}
...
public static class FixtureResults
{
    ...
    public static int TotalCount(this IEnumerable<FixtureResult> @this)
        => @this.FixtureResultCount(_ => true);
    ...
    public static int PassedCount(this IEnumerable<FixtureResult> @this)
        => @this.FixtureResultCount(result => result.Status is FixtureStatus.Passed);
    ...
    public static int FailedCount(this IEnumerable<FixtureResult> @this)
        => @this.FixtureResultCount(result => result.Status is FixtureStatus.Failed);
    ...
    public static int PendingCount(this IEnumerable<FixtureResult> @this)
        => @this.FixtureResultCount(result => result.Status is FixtureStatus.Pending);
    ...
    public static DateTime StartTime(this IEnumerable<FixtureResult> @this)
        => @this.Select(result => result.StartTime)
            .OfType<DateTime>()
    ...
    public static DateTime EndTime(this IEnumerable<FixtureResult> @this)
    {
        ...
        return results.IsEmpty() ? DateTime.MinValue :
            results.Select(result => result.EndTime)
                .OfType<DateTime>()
    ...
    {
        var results = @this.ToList();
        return results.Where(result => result.Results.IsEmpty()).Count(filter) +
            results.Sum(result => result.Results.FixtureResultCount(filter));
    }
}