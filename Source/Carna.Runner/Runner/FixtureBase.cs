...
public abstract class FixtureBase : IFixture
{
    ...
    /// The <see cref="FixtureAttribute"/> is not specified to the <paramref name="fixtureInstanceType"/>.
    /// </exception>
    protected FixtureBase(Type fixtureInstanceType) : this(fixtureInstanceType, fixtureInstanceType.Name, fixtureInstanceType.FullName ?? fixtureInstanceType.Name, fixtureInstanceType.GetTypeInfo().GetCustomAttribute<FixtureAttribute>() ?? throw new ArgumentException($"FixtureAttribute is not specify to {fixtureInstanceType}."))
    {
    }
    ...
    /// <exception cref="ArgumentException">
    /// The <see cref="FixtureAttribute"/> is not specified to the <paramref name="fixtureMethod"/>.
    /// </exception>
    protected FixtureBase(Type fixtureInstanceType, MethodInfo fixtureMethod) : this(fixtureInstanceType, fixtureMethod, fixtureMethod.GetCustomAttribute<FixtureAttribute>() ?? throw new ArgumentException($"FixtureAttribute is not specified to {fixtureMethod}."))
    {
    }
    ...
    /// <param name="fixtureMethod">The fixture method.</param>
    /// <param name="attribute">The attribute that specifies a fixture.</param>
    protected FixtureBase(Type fixtureInstanceType, MethodInfo fixtureMethod, FixtureAttribute attribute) : this(fixtureInstanceType, fixtureMethod.Name, $"{fixtureInstanceType.FullName}.{fixtureMethod.Name}", attribute)
    {
    }
    ...
    /// <param name="attribute">The attribute that specifies a fixture.</param>
    protected FixtureBase(Type fixtureInstanceType, string name, string fullName, FixtureAttribute attribute) : this(fixtureInstanceType, new FixtureDescriptor(name, fullName, attribute))
    {
    }
    ...
    protected virtual bool CanRun(IFixtureFilter? filter)
        => filter is null || filter.Accept(FixtureDescriptor);
    ...
    /// <returns>The result of a fixture running.</returns>
    ...
    protected virtual bool IsRunningInSta() => Thread.CurrentThread.GetApartmentState() is ApartmentState.STA;
    ...
    {
        if (FixtureInstanceType is null) return null;

        var constructor = GetConstructor();
        if (constructor is null)
        {
            var fixtureInstance = Activator.CreateInstance(FixtureInstanceType);
            return fixtureInstance is null ? fixtureInstance : EnsureParameters(fixtureInstance);
        }

        var constructorParameters = constructor.GetParameters();
        var parameterValues = new object?[constructorParameters.Length];
        if (ParentFixture is not null)
        {
            Enumerable.Range(0, constructorParameters.Length).ForEach(index =>
            {
                var parameterName = constructorParameters[index].Name;
                if (parameterName is not null && ParentFixture.Parameters.ContainsKey(parameterName))
                {
    ...
    {
        if (FixtureInstanceType is null) return null;
        if (ParentFixture is null || ParentFixture.Parameters.IsEmpty()) return null;
    ...
    {
        if (FixtureInstanceType is null) return string.Empty;
    ...
    protected virtual List<BackgroundAttribute> RetrieveBackgroundAttributes(MethodBase? constructor)
    {
        ...
        if (constructor is null || constructor.DeclaringType == typeof(object)) return backgroundList;
